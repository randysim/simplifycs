...
export default function ArticlesList({ selectArticle, selectedArticle }) {
  ...
  function deleteArticle(article) {
    axios
      .delete("/api/articles/editor/updateArticle", {
        data: {
          id: article.id,
        },
      })
  ...
  function updateArticleTitle(article, newTitle) {
    axios
      .post("/api/articles/editor/updateArticle", {
        id: article.id,
        data: {
          title: newTitle,
        },
  ...
  function createNewArticle() {
    axios.post("/api/articles/editor/createNewArticle").then(() => {
      mutate();
    });
  }
    ...
    <div className={styles.articleList}>
      ...
      {(data || []).map((article, i) => (
        <div
          style={{
            display: "flex",
            background: article.id == selectedArticle ? "#00ff00" : "#ff0000",
          }}
          key={i}
        >
          <input
            ...
            onChange={(event) => {
              updateArticleTitle(article, event.target.value);
            }}
          />
