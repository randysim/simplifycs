...
import { Box, Typography, TextField, Snackbar, Grid, Button } from "@mui/material";
...
export default function AdminUnit() {
    ...
    const Save = async () => {
        axios.post("/api/admin/editunit", { id: unitid, data: unitData })
            .then(res => {
                if (res.data.success) {
                    getCourseData(courseid)
                    .then((crs) => {
        ...
        return (
            <Box>
                ...
                <Box sx={{ display: "flex", width: "100%", flexWrap: "wrap", height: "auto", padding: "50px"}}>
                    <Box sx={{ width: "100%"}}>
                        <TextField sx={{ width: "80%" }} id="outlined-basic" label="Title" variant="outlined" value={unitData.title} onChange={e => {
                            setUnitData({ ...unitData, title: e.target.value })
                            setSavable(true); 
                        }} />
                    </Box>
                    <Box sx={{ width: "100%", marginTop: "20px"}}>
                        <TextField sx={{ width: "80%" }} id="outlined-basic" label="Description" variant="outlined" value={unitData.description} multiline rows={5} onChange={e => {
                    ...
                    <Grid
                        ...
                        sx={{ width: "100%", height: "auto", padding: "50px", marginTop: "40px", borderRadius: "10px" }}
                        bgcolor="#AF98B9"
                    >
                        {unitData.lessons.map(lesson => {
                            return (
                                <AdminLessonCard 
                                    ...
                                    onDelete={() => {
                                        setUnitData({ ...unitData, lessons: unitData.lessons.filter(l => l.id != lesson.id)});
                                        setSavable(true);
                                    }}
                                    onTitleChange={e => {
                                        setUnitData(
                                        ...
                                        setUnitData(
                                            { 
                                                ...
                                                lessons: [
                                                    ...
                                                    { 
                                                        ...lesson, 
                                                        activities: lesson.activities.filter(a => a.id != id) 
                                                    }
                                                ]
                                            }
                                        );
                    ...
                    <Button variant="outlined" sx={{ width: "80%", marginTop: "20px"}} onClick={() => {setOpen(true)}} >Add Lesson</Button>
                    </Grid>
                </Box>
                ...
                <ConfirmationDialog 
                    ...
                    open={open}
                    onConfirm={() => {
                        setUnitData({ ...unitData, lessons: [...unitData.lessons, { title: `PendingLesson-${genAuthToken()}`, activities: [] }]})
                        setOpen(false);
                        setSavable(true);
                    }}
                    onClose={() => {
